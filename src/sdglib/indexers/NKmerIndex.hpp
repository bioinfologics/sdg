...
public:
    ...
    void generate_index(const SequenceDistanceGraph &sg, int filter_limit = 200, bool verbose=true) {
        ...
        if (verbose) sdglib::OutputLog() << "Filtering kmers appearing less than " << max_kmer_repeat << " from " << assembly_kmers.size() << " initial kmers" << std::endl;
        ...
        if (verbose) sdglib::OutputLog() << "Kmers for mapping " << assembly_kmers.size() << std::endl;
