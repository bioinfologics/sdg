...
#include <sdglib/filesystem/helpers.hpp>
...
public:
    ...
    SMR(ReaderParamStruct reader_parameters, FactoryParamStruct factory_parameters, SMRParams smr_params) : reader_parameters(reader_parameters),
        ...
        sdglib::check_or_create_directory(this->outdir);
        numElementsPerBatch = (smr_params.maxMem / sizeof(RecordType) / maxThreads);
        sdglib::OutputLog(sdglib::DEBUG)<<"SMR created with elements of size "<<sizeof(RecordType)<<" using "<<maxThreads<<" threads and "
    ...
    std::vector<RecordType> read_from_file(const std::string &read_file, const bool do_work=true) {
        ...
        sdglib::check_or_create_directory(finalFilePath);
        outdir = finalFilePath;
        tmpInstance = sdglib::create_temp_directory(tmpBase);
        tmpInstance+="/";
        std::ifstream final_file(finalFilePath+"final.kc");
        if (final_file.is_open() and !do_work){
            sdglib::OutputLog(sdglib::DEBUG) << "Using precomputed sum file at " << outdir << "final.kc" << std::endl;
            ...
            sdglib::OutputLog(sdglib::DEBUG) << "Reading file: " << read_file << std::endl;
            FileReader myFileReader(reader_parameters, read_file);
            sdglib::OutputLog(sdglib::DEBUG) << "Begin reduction using " << numElementsPerBatch << " elements per batch (" << ceil(uint64_t((numElementsPerBatch*sizeof(RecordType)*maxThreads)) / (1.0f*1024*1024*1024)) << "GB)" << std::endl;
            sdglib::OutputLog(sdglib::DEBUG) << "Using tmp: " << tmpInstance << std::endl;
            ...
            sdglib::OutputLog(sdglib::DEBUG) << "Done reduction in " << elapsed_seconds.count() << "s" << std::endl;
            auto result(getRecords());
            sdglib::remove_directory(tmpInstance);
            return result;
        }
    };
    ...
    std::vector<RecordType> process_from_memory(const bool do_work = true) {
        ...
        sdglib::OutputLog(sdglib::DEBUG) << "Reading From memory" << std::endl;
        FileReader myFileReader(reader_parameters);
        sdglib::OutputLog(sdglib::DEBUG) << "Begin reduction using " << numElementsPerBatch << " elements per batch ("
...
private:
    ...
    uint64_t merge(const std::string &tmpName, const std::vector<std::string> &files, std::vector<RecordType> &elements) {
        ...
        for (auto i = 0; i < files_size; i++) {
            ...
            //sdglib::OutputLog(sdglib::DEBUG) << files[i] << " size " << size << " in disk " << sizeof(size) + size* sizeof(RecordType) << std::endl;
