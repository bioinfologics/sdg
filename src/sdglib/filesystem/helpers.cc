...
bool sdglib::check_file(std::string &filepath) {
    struct stat sb{};
...
bool sdglib::check_or_create_directory(std::string &output_prefix) {
...
void sdglib::remove_directory(std::string path) {
    std::cout << "Removing: " << path << std::endl;
    ::rmdir(path.c_str());
}

std::string sdglib::create_temp_directory(std::string prefix = "/tmp") {
    std::string tmplt (
            (prefix.empty()) ?
...
bool sdglib::copy_file(const std::string& from_p, const std::string& to_p, bool fail_if_exists)
{
