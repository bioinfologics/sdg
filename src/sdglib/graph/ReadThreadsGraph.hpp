...
public:
    ...
    /**
     * Get the NodeView at the start of the thread
     * @param thread_id id of the thread
     * @return nodeview of the start of the thread
     */
    NodeView thread_start_nodeview(int64_t thread_id);

    /**
     * Get the NodeView at the end of the thread
     * @param thread_id id of the thread
    ...
    /**
     * Get the next link from a node in a thread, if there is more than one or no links throws an exception
     *
    ...
    /**
     * Same as next_in_thread but with prev
     * @param nid
     * @param thread_id
     * @param link_index
     * @return
    ...
    /**
     * From a thread id get all node ids starting from a node.
    ...
    /**
     * Construct the vecinity thread graph starting from a node
    ...
    /**
     * Get all threads that go over the node
    ...
    /**
     * Removes the selected node from the selected thread in the graph (inplace).
    ...
    /**
     * Removes a group of nodes from a thread in the graph (inplace)
    ...
    /**
     * Pop the node from all the threads in the graph (inplace). (the pop_node function was executed for all
    ...
    /**
     * Get a list of node positions for the thread
     * @param thread_id thread id
