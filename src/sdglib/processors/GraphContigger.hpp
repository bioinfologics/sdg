...
public:
    ...
    std::map<sgNodeID_t,std::vector<std::pair<sgNodeID_t, int64_t>>> contig_reduction_to_unique_kmers(WorkSpace & ws, std::string kmer_counter, std::string kmer_count, int min_cov, int max_cov, uint32_t max_run_size=100, float min_kci=0, float max_kci=0);
