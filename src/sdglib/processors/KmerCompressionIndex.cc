...
void KmerCompressionIndex::index_graph(){
    ...
    sdglib::OutputLog(sdglib::INFO)<<graph_kmers.size()<<" kmers in total"<<std::endl;
    sdglib::OutputLog(sdglib::INFO) << "  Sorting..."<<std::endl;
    sdglib::sort(graph_kmers.begin(),graph_kmers.end());
    sdglib::OutputLog(sdglib::INFO) << "  Merging..."<<std::endl;
    ...
    sdglib::OutputLog(sdglib::INFO)<<graph_kmers.size()<<" kmers in index"<<std::endl;
...
void KmerCompressionIndex::reindex_graph(){
    sdglib::OutputLog(sdglib::INFO) << "Re-indexing graph, Counting..."<<std::endl;
    ...
    KmerCountFactory<FastaRecord>kcf({k});
    for (sgNodeID_t n=1;n<sg.nodes.size();++n){
        if (n%10000 == 0) sdglib::OutputLog(sdglib::INFO) << "Node id: " << n << "/" << sg.nodes.size() << std::endl;
    ...
    sdglib::OutputLog(sdglib::INFO) << "  Updating counts..."<<std::endl;
    for (auto i=0;i<graph_kmers.size();++i){
        graph_kmers[i].count=new_counts[i];
    }
    sdglib::OutputLog(sdglib::INFO) << "Re-indexing done."<<std::endl;
}
...
void KmerCompressionIndex::add_counts_from_file(std::vector<std::string> filenames) {
    ...
    sdglib::OutputLog(sdglib::INFO)<<"Map populated, processing files"<<std::endl;
...
void KmerCompressionIndex::compute_compression_stats() {
    ...
    sdglib::OutputLog()<<"KCI Mean coverage for unique kmers:   " << ((double)tuniq)/cuniq <<std::endl;
    sdglib::OutputLog()<<"KCI Median coverage for unique kmers: " << median <<std::endl;
    sdglib::OutputLog()<<"KCI Mode coverage for unique kmers:   " << mode <<std::endl;

    if ( std::abs( float(median-mode) ) > mode*.1) sdglib::OutputLog()<<"WARNING -> median and mode highly divergent"<<std::endl;
    uniq_mode=mode;
...
void KmerCompressionIndex::add_counts_from_datastore(const PairedReadsDatastore &ds) {
    ...
    kmer_map.reserve(graph_kmers.size());
    for (uint64_t i=0;i<graph_kmers.size();++i) kmer_map[graph_kmers[i].kmer]=i;
    sdglib::OutputLog(sdglib::INFO)<<"Map populated, counting from datastore: " << ds.filename << std::endl;
