...
#include <sdglib/processors/PathExplorer.hpp>
#include <sdglib/utilities/nstats.hpp>
...
void MappingThreader::thread_mappings() {
    sdglib::OutputLog() << "Assembling mappings into contiguous threads" << std::endl;
    ...
    while (c) {
        sdglib::OutputLog() << "Threading mappings for " << refsequence.name << std::endl;
        ...
        for(const auto &sm : mappings) {
            ...
            sdglib::OutputLog(sdglib::DEBUG) << "Considering the mapping:" << std::endl << getGraph().nodeID_to_name(sm.absnode()) << std::endl;
            sdglib::OutputLog(sdglib::DEBUG) << "Trying to append mapping to current thread trivially" << std::endl;
            ...
            if(could_append_trivially) {
                sdglib::OutputLog(sdglib::DEBUG) << "Appended mapping to current thread" << std::endl;
            } else {
                sdglib::OutputLog(sdglib::DEBUG) << "Failed to append mapping to current thread trivially" << std::endl;
                sdglib::OutputLog(sdglib::DEBUG) << "Trying to append mapping to current thread by path finding" << std::endl;
                ...
                if (found_path) {
                    sdglib::OutputLog(sdglib::LogLevels::DEBUG) << "Found a suitable path connecting the two mappings" << std::endl;
                    sdglib::OutputLog(sdglib::LogLevels::DEBUG) << "Adding the nodes of this suitable path to the sequence mapping thread" << std::endl;
                    sdglib::OutputLog(sdglib::LogLevels::DEBUG) << "Current bestpath: ";
                    ...
                    sdglib::OutputLog(sdglib::LogLevels::DEBUG) << "Adding nodes of bestpath to the current thread:" << std::endl;
                    ...
                    sdglib::OutputLog(sdglib::LogLevels::DEBUG) << "Could not trivially link mappings, or get there through path search" << std::endl;
                    sdglib::OutputLog(sdglib::LogLevels::DEBUG) << "Saving current path." << std::endl;
                    mappingThreads.emplace_back(mappingThread);
                    sdglib::OutputLog(sdglib::LogLevels::DEBUG) << "Clearing path to start a new path." << std::endl;
                    mappingThread.clear();
                    sdglib::OutputLog(sdglib::LogLevels::DEBUG) << "Adding node to new path." << std::endl;
                    append_mapping_trivially(mappingThread, sm);
                }
            }
            sdglib::OutputLog(sdglib::LogLevels::DEBUG) << "Current path of mappings is " << mappingThread.size() << " nodes long." << std::endl;
        }
        sdglib::OutputLog() << "Finished connecting paths for sequence " << refsequence.name << std::endl;
        mappingThreads.emplace_back(mappingThread);
        mappingThread.clear();
...
bool MappingThreader::append_mapping_trivially(MappingThread &thread, NodeMapping mapping) {
    ...
    sdglib::OutputLog(sdglib::LogLevels::DEBUG) << "Trying to add to path trivially as: " << dn << std::endl;
    auto pathSuccess = thread.nodePath().append_to_path(dn);
    if (pathSuccess) {
        sdglib::OutputLog(sdglib::LogLevels::DEBUG) << "Was able to trivially append " << dn << " to the path." << std::endl;
        thread.orderedMappings().emplace_back(mapping);
    } else {
        sdglib::OutputLog(sdglib::LogLevels::DEBUG) << "Was not able to trivially append " << dn << " to the path." << std::endl;
    }
    return pathSuccess;
}
...
bool MappingThreader::append_mapping_with_path(MappingThread &thread, NodeMapping mapping, SequenceGraphPath &path) {
    for (auto node = ++path.getNodes().begin(); node != path.getNodes().end(); ++node) {
        sdglib::OutputLog(sdglib::LogLevels::DEBUG) << "Adding node: " << *node << std::endl;
        auto status = thread.node_path.append_to_path(*node);
        sdglib::OutputLog(sdglib::LogLevels::DEBUG) << status << std::endl;
        if (!status) {
