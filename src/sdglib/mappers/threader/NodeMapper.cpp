...
NodeMapper::NodeMapper(SequenceDistanceGraph &sg, UniqueKmerIndex &uki) : sg(sg), graph_kmer_index(uki) {}
...
void NodeMapper::write_mappings_to_binary_file(std::string filename) const {
    sdglib::OutputLog() << "Dumping node mappings" << std::endl;
...
void NodeMapper::read_mappings_from_binary_file(std::string filename) {
    sdglib::OutputLog() << "Loading node mappings" << std::endl;
...
void NodeMapper::filterMappings(double score) {
    sdglib::OutputLog(sdglib::LogLevels::INFO) << "Filtering mappings using a threshold of " << score << "%" << std::endl;
...
void NodeMapper::map_sequences_from_file(const std::string &filename) {
    sdglib::OutputLog(sdglib::LogLevels::INFO) << "Mapping sequence kmers to graph" << std::endl;
...
#pragma omp critical (getrecord)
        ...
        while (c) {
            ...
            sdglib::OutputLog(sdglib::LogLevels::INFO) << "Mapping sequence: " << sequence.name << " (" << sequence.id << ')' << std::endl;
            std::tie(mappings, unmapped_kmers) = map_sequence_to_graph(sequence);

#pragma omp critical (store)
            {
                sdglib::OutputLog(sdglib::LogLevels::INFO) << "Failed to map " << unmapped_kmers.size() << " kmers from the reference" << std::endl;
...
std::tuple<std::vector<NodeMapping>, std::vector<KmerIDX>> NodeMapper::map_kmers_to_graph(seqID_t id, std::vector<KmerIDX>& kmers) {
    ...
    for (auto &sk:kmers) {
        ...
        if (found_kmer) {
            sdglib::OutputLog(sdglib::LogLevels::DEBUG) << "Found kmer: " << sk.kmer << "in graph" << std::endl;
            sdglib::OutputLog(sdglib::LogLevels::DEBUG) << '(' << sg.nodeID_to_name(graph_pos.node) << ", " << graph_pos.pos << ')' << std::endl;
            mapped_kmers_count++;
            // IF THE KMER MATCH IS THE FIRST MATCH FOR THE MAPPING...
            if (!mapping.ismatched()) {
                sdglib::OutputLog(sdglib::LogLevels::DEBUG) << "Kmer match is the first for the mapping." << std::endl;
                start_new_mapping(mapping, graph_pos, sk.pos);
            } // IF THE KMER MATCH IS NOT THE FIRST MATCH FOR THE MAPPING...
            else {
                sdglib::OutputLog(sdglib::LogLevels::DEBUG) << "Kmer match is not the first match for the mapping." << std::endl;
                ...
                if (!mapping.mapping_continues(graph_pos)) {
                    sdglib::OutputLog(sdglib::LogLevels::DEBUG) << "Kmer match does not continues the current mapping." << std::endl;
                    sequence_mappings.push_back(mapping);
                    start_new_mapping(mapping, graph_pos, sk.pos);
                } else { // IF NODE KMER MAPS TO IS THE SAME, EXTEND THE CURRENT MAPPING...
                    sdglib::OutputLog(sdglib::LogLevels::DEBUG) << "Kmer match continues the current mapping." << std::endl;
    ...
    sdglib::OutputLog(sdglib::LogLevels::INFO) << "Mapped " << mapped_kmers_count << " kmers from the reference." << std::endl;
