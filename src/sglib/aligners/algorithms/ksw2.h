//
// Created by Ben Ward (EI) on 10/04/2018.
// Copied from: https://github.com/lh3/ksw2
...
int kswalign(const char *target, const char *query, int matchScore, int mismatchScore, int gapOpen, int gapExtend) {
    int i;
    int8_t a = matchScore, b = mismatchScore < 0? mismatchScore : -mismatchScore; // a>0 and b<0
    int8_t mat[25] = { a,b,b,b,0, b,a,b,b,0, b,b,a,b,0, b,b,b,a,0, 0,0,0,0,0 };
    int targetlen = strlen(target), querylen = strlen(query);
    uint8_t *ts, *qs, c[256];
    ksw_extz_t ez;
    ...
    ts = (uint8_t*)malloc(targetlen);
    qs = (uint8_t*)malloc(querylen);
    for (i = 0; i < targetlen; ++i) ts[i] = c[(uint8_t)target[i]]; // encode to 0/1/2/3
    for (i = 0; i < querylen; ++i) qs[i] = c[(uint8_t)query[i]];
    ksw_extz2_sse(0, querylen, qs, targetlen, ts, 5, mat, gapOpen, gapExtend, -1, -1, 0, &ez);
    const int out = ez.score;
